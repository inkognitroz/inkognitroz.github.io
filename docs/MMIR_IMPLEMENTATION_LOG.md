...
- D207 is now next: harden free live-model route selection so first-time users always see useful ready-now or installable free model paths before paid/provider routes.
...
- Validate latest frontend/backend/local-node CI after the new commits, then continue with `D207` free live-model route hardening.
