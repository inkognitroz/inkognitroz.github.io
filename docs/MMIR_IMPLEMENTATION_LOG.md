...
- D172 is now the next activation slice: when autopilot cannot fix a connector/Ollama/model/pairing gap, the UI should show one exact OS/device repair card.
...
- Validate latest frontend/backend/local-node CI after the new commits, then continue with `D172` guided OS/device repair cards.
