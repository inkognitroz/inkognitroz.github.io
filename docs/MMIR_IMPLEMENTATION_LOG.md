...
- D288 is now beta: the composer plus button now opens an Open WebUI-style quick actions drawer for Models, Install node, Knowledge, New chat, Voice and Settings, with each action wired to existing free/local or protected panels and no paid/provider side effects.
- D289 is now beta: the quick actions drawer now starts with `Ready now` route status and a primary `Chat now` action that seeds a safe free prompt only when the composer is empty, keeping setup optional.
- D290 is now beta: the quick actions drawer now includes direct free route chips for MMIR Guide, Browser WebGPU and Qwen3 local install, dispatching starter handoff or installer resume without opening a heavier picker first.
- D291 is now beta: a dependency-free quick-route click fixture now opens the composer tools drawer and clicks Guide, Browser WebGPU and Local install routes to prove prompt seeding, starter handoff, send click, installer resume and no-spend/no-secret boundaries in CI.
- D292 is now beta: managed free API/local profile defaults now run from a small critical script while the heavier backend settings UI loads progressively, restoring first-paint JS headroom without removing the automatic free route.
- D293 is now beta: the progress dashboard backlog now renders in a bounded first window with a Show all control and debounced filtering, so deep-linking to the owner view cannot flood the DOM and make the chat-first page feel stuck.
- D294 is now beta: the first-paint composer now shows a compact fallback `+` and circular arrow send button, then hides the fallback plus when the richer composer dock is installed, preserving Open WebUI-style feel without duplicate controls.
- D295 is now beta: critical runtime CSS now keeps the pre-chat composer dock alive with only the `+` visible, so the Add Model/tools entry does not disappear while deferred quick-actions styling loads.
- D296 is now beta: runtime model-helper and installer-status styling moved into deferred CSS, blocking CSS now has a tighter 67 KB guard, the service worker cache rotated and deferred quick-actions CSS no longer carries the old whole-dock hide race.
...
- D301 is now beta: active-node rail polish moved from critical JavaScript into deferred runtime CSS, leaving only a minimal layout guard in the startup script and restoring first-paint JS headroom.
- D303 is now beta: the no-spend catalog now includes 28 free startup chat starters plus additional free local RAG starter metadata, and the active node rail can connect LM Studio, llama.cpp and vLLM as local OpenAI-compatible profiles while routing native Ollama Direct users through the safer MMIR Local Node bridge.
- D304 is now beta: LM Studio, llama.cpp and vLLM adapter clicks now hand off through the chat-runtime bridge, refresh live `/v1/models`, select a real local model before chat and show a clear local-server/CORS hint instead of silently falling back to the browser guide.
- D305 is now beta: `mmir-local-node` commit `393ce65` makes Windows installers prefer `npm.cmd`, updates PowerShell docs, adds a regression test for the `npm.ps1` execution-policy trap, and keeps tests, lint, secret scan, node conformance and full installer dry-run green.
- D306 is now beta: the public local connector now always probes the fixed localhost Local Node URL instead of the active managed profile, remembers successful local-node proof in a time-limited secret-free marker, and lets the next visit auto-check live local models without broad localhost probing.
- D307 is now beta: live Local Node models are promoted directly into the active chat route headline, and the Local Node action now refreshes through chat-runtime and sends exactly once with a private model-specific prompt.
