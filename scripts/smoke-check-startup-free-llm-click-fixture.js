...
liveLocal.window.dispatchEvent(new FakeCustomEvent('mmir-local-connector-refreshed', {
  detail: { status: 'offline', url: 'http://127.0.0.1:3000', models: [] }
}));
liveLocal.window.dispatchEvent(new FakeCustomEvent('mmir-local-connector-refreshed', {
  detail: { url: 'http://127.0.0.1:3000', models: [{ id: 'qwen3:0.6b', name: 'Qwen3 0.6B' }] }
}));
...
requireIncludes(text(files.backlog), '| D310 | Chat UX / Local Node | P0 | Local node ready-status tolerance |', 'Backlog must include D310 local node ready-status tolerance.');
requireIncludes(text(files.backlog), '| D313 | Chat UX / Local Node | P0 | Fresh local model clears stale offline status |', 'Backlog must include D313 stale offline model proof.');
...
requireIncludes(text(files.log), 'D310 is now beta', 'Implementation log must include D310.');
requireIncludes(text(files.log), 'D313 is now beta', 'Implementation log must include D313.');
...
requireIncludes(text(files.buildDashboard), "['D310'", 'Progress dashboard build must mark D310 status.');
requireIncludes(text(files.buildDashboard), "['D313'", 'Progress dashboard build must mark D313 status.');
...
requireIncludes(text(files.visualQa), 'D310 local node ready-status tolerance', 'Visual QA report must mention D310.');
requireIncludes(text(files.visualQa), 'D313 stale offline local model recovery', 'Visual QA report must mention D313.');
