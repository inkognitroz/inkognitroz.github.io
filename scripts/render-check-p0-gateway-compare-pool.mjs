...
const preferredPort = Number(process.env.MMIR_GATEWAY_COMPARE_RENDER_PORT || 8796);
...
async function resolveGatewayCompareRenderPort() {
  const maxAttempts = Number(process.env.MMIR_GATEWAY_COMPARE_RENDER_PORT_ATTEMPTS || 50);
  for (let offset = 0; offset < maxAttempts; offset += 1) {
    const candidate = preferredPort + offset;
    if (await canListen(candidate)) return candidate;
  }
  throw new Error(`No available gateway compare render QA port found from ${preferredPort} across ${maxAttempts} attempts`);
}
...
port = await resolveGatewayCompareRenderPort();
