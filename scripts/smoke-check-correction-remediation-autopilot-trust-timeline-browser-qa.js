...
requireIncludes(files.backlog, '| D252 |', 'Backlog must add D252 after D249.');
...
const d250 = tasks.find((task) => task.seq === 'D252');
requireTrue(d249?.status === 'beta', 'Progress dashboard task D249 must be beta after browser QA ships.');
requireTrue(d250?.status === 'next', 'Progress dashboard task D252 must become next after D249 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D252', 'Progress dashboard next queue must prioritize D252 after D249 ships.');
