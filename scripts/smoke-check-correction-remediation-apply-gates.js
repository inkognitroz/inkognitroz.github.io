...
requireIncludes(files.backlog, '| D240 |', 'Backlog must add D240 after D237.');
requireIncludes(files.implementationLog, 'D237 is now beta', 'Implementation log must mark D237 beta.');
requireIncludes(files.implementationLog, 'D240 is now next', 'Implementation log must mark D240 next.');
...
const d238 = tasks.find((task) => task.seq === 'D240');
requireTrue(d237?.status === 'beta', 'Progress dashboard task D237 must be beta after apply gates ship.');
requireTrue(d238?.status === 'next', 'Progress dashboard task D240 must become next after D237 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D240', 'Progress dashboard next queue must prioritize D240 after D237 ships.');
