...
assert(mmir.includes('p0-release-nav.js?v=20260918-jarvis-v2'),'chat must load the release navigation script');
...
assert(mmir.includes('p0-chat-shell.js?v=20260914-public-failure-route-details-v1'),'chat must bind the reviewed public failure route-details shell asset version');
...
assert(p0Shell.includes('Grunnchat kan prøves.')&&p0Shell.includes('Status og avanserte kvalitetsporter er ikke bekreftet.')&&p0Shell.includes('Ikke del sensitiv info eller bruk til høyrisikoformål.'),'basic chat must keep attemptability, unverified-status, privacy and high-risk truth separate');
...
assert(runtime.includes('release?.authenticatedReleaseReady===true')&&runtime.includes('releaseReady&&verified>0'),'trust green must consume full authenticated release authority, not singleton first-chat availability');
