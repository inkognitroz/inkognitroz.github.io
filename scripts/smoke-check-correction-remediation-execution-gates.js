...
requireIncludes(files.backlog, '| D244 |', 'Backlog must add D244 after D240.');
...
const d241 = tasks.find((task) => task.seq === 'D244');
requireTrue(d240?.status === 'beta', 'Progress dashboard task D240 must be beta after execution gates ship.');
requireTrue(d241?.status === 'next', 'Progress dashboard task D244 must become next after D240 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D244', 'Progress dashboard next queue must prioritize D244 after D240 ships.');
