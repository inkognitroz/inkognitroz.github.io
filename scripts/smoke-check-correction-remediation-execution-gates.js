...
requireIncludes(files.backlog, '| D250 |', 'Backlog must add D250 after D240.');
...
const d241 = tasks.find((task) => task.seq === 'D250');
requireTrue(d240?.status === 'beta', 'Progress dashboard task D240 must be beta after execution gates ship.');
requireTrue(d241?.status === 'next', 'Progress dashboard task D250 must become next after D240 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D250', 'Progress dashboard next queue must prioritize D250 after D240 ships.');
