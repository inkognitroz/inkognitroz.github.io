...
requireIncludes(files.backlog, '| D247 |', 'Backlog must add D247 after D240.');
...
const d241 = tasks.find((task) => task.seq === 'D247');
requireTrue(d240?.status === 'beta', 'Progress dashboard task D240 must be beta after execution gates ship.');
requireTrue(d241?.status === 'next', 'Progress dashboard task D247 must become next after D240 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D247', 'Progress dashboard next queue must prioritize D247 after D240 ships.');
