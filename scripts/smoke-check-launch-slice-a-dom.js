...
requireIncludes(files.mmir, 'p0-chat-shell.css?v=20260602-p0-mention-compare-v12', 'P0 simple chat shell CSS must load on the public page.');
requireIncludes(files.mmir, 'p0-chat-shell.js?v=20260602-p0-mention-compare-v12', 'P0 simple chat shell runtime must load on the public page.');
...
requireIncludes(files.p0Runtime, "state.models.find(model=>model.route==='local')", 'P0 compare must stay gated behind real local model discovery.');
...
requireIncludes(files.p0Runtime, 'Tiny private model · fast demo · weak factual recall', 'Tiny local models must be labeled as weak factual recall.');
...
requireIncludes(files.p0Runtime, "const HISTORY_SCHEMA='20260601-clean-first-chat-v1'", 'P0 shell must invalidate stale browser-error chat history.');
