...
requireIncludes(files.p0Runtime, 'data-p0-route-action="connect-local"', 'P0 shell must keep contextual local setup available without settings clutter.');
...
requireIncludes(files.p0Runtime, "action==='role-profile-menu'", 'P0 role-profile capability must remain available without cluttering settings.');
...
requireIncludes(files.p0Runtime, 'if(partner&&wantsCompareRoute(prompt))', 'P0 compare must activate from user intent after route discovery.');
...
requireIncludes(files.p0Runtime, "function renderReceipt(receipt,proof,modelLabel='',intelligenceLabel='',answerState='',aiGenerated=false,answerWriter=null,failureDiagnostic=null)", 'P0 route receipts must keep full audit details available on click.');
