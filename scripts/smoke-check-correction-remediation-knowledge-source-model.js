...
requireIncludes(files.backlog, '| D250 |', 'Backlog must add D250 after D242.');
...
const d243 = tasks.find((task) => task.seq === 'D250');
requireTrue(d242?.status === 'beta', 'Progress dashboard task D242 must be beta after knowledge source model ships.');
requireTrue(d243?.status === 'next', 'Progress dashboard task D250 must become next after D242 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D250', 'Progress dashboard next queue must prioritize D250 after D242 ships.');
