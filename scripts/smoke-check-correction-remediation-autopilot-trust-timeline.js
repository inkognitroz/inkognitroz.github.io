...
requireIncludes(files.backlog, '| D252 |', 'Backlog must add D252 after D248.');
...
const d249 = tasks.find((task) => task.seq === 'D252');
requireTrue(d248?.status === 'beta', 'Progress dashboard task D248 must be beta after trust timeline ships.');
requireTrue(d249?.status === 'next', 'Progress dashboard task D252 must become next after D248 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D252', 'Progress dashboard next queue must prioritize D252 after D248 ships.');
