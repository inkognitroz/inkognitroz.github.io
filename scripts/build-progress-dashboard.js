...
const overrides = new Map([
  ...
  ...range(116, 195).map((id) => [id, { status: 'planned', evidence: 'Added to the expanded GUI parity and platform-readiness backlog; implementation has not started.' }]),
  ...
  ['D195', { status: 'next', evidence: 'Next automation slice: after repair returns, offer one safe retry or auto-continue only when the local node proves ready.' }]
]);
...
const prioritizedNextIds = ['D195', 'D117', 'D116', 'D118', 'D119'];
