...
const overrides = new Map([
  ...
  ...range(116, 184).map((id) => [id, { status: 'planned', evidence: 'Added to the expanded GUI parity and platform-readiness backlog; implementation has not started.' }]),
  ...
  ['D184', { status: 'next', evidence: 'Next activation UX slice: surface the live activation closure checklist on the first screen so users do not need the owner dashboard to finish setup.' }]
]);
...
const prioritizedNextIds = ['D184', 'D117', 'D116', 'D118', 'D119'];
