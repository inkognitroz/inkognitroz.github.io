...
const overrides = new Map([
  ...
  ...range(116, 174).map((id) => [id, { status: 'planned', evidence: 'Added to the expanded GUI parity and platform-readiness backlog; implementation has not started.' }]),
  ...
  ['D174', { status: 'next', evidence: 'Next activation slice: wire repair-card actions to the exact panel/installer/model-library flow and record the chosen path in activation telemetry.' }]
]);
...
const prioritizedNextIds = ['D174', 'D117', 'D116', 'D118', 'D119'];
