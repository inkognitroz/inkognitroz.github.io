...
const overrides = new Map([
  ...
  ...range(116, 191).map((id) => [id, { status: 'planned', evidence: 'Added to the expanded GUI parity and platform-readiness backlog; implementation has not started.' }]),
  ...
  ['D191', { status: 'next', evidence: 'Next first-screen UX slice: add one safe continue action from the first-screen starter progress strip to the exact missing step.' }]
]);
...
const prioritizedNextIds = ['D191', 'D117', 'D116', 'D118', 'D119'];
