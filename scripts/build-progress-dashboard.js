...
const overrides = new Map([
  ...
  ...range(116, 160).map((id) => [id, { status: 'planned', evidence: 'Added to the expanded GUI parity and platform-readiness backlog; implementation has not started.' }]),
  ...
  ['D160', { status: 'next', evidence: 'Next sharing/security slice: make owner-side team sharing easier with a safe handoff packet that pre-fills non-secret recipient fields while one-time codes remain one-time.' }]
]);
...
const prioritizedNextIds = ['D160', 'D117', 'D116', 'D118', 'D119'];
