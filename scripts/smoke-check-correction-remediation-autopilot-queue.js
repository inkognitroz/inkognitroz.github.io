...
requireIncludes(files.backlog, '| D252 |', 'Backlog must add D252 after D245.');
...
const d246 = tasks.find((task) => task.seq === 'D252');
requireTrue(d245?.status === 'beta', 'Progress dashboard task D245 must be beta after autopilot ships.');
requireTrue(d246?.status === 'next', 'Progress dashboard task D252 must become next after D245 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D252', 'Progress dashboard next queue must prioritize D252 after D245 ships.');
