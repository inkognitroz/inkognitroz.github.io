...
for (const needle of [
  'composer-quick-actions.css?v=20260526-stale-offline-model-proof-v1',
  'composer-quick-actions.js?v=20260526-stale-offline-model-proof-v1'
]) {
...
requireIncludes(text(files.visualQa), 'D313 stale offline local model recovery', 'Visual QA report must mention D313 stale offline local model recovery.');
...
requireIncludes(text(files.backlog), '| D313 | Chat UX / Local Node | P0 | Fresh local model clears stale offline status |', 'Backlog must include D313.');
...
const d313 = (progress.tasks || []).find((task) => task.seq === 'D313');
if (!d313 || d313.status !== 'beta') fail('Progress dashboard task D313 must be beta.');
