...
requireIncludes(files.backlog, '| D242 |', 'Backlog must add D242 after D239.');
requireIncludes(files.implementationLog, 'D239 is now beta', 'Implementation log must mark D239 beta.');
requireIncludes(files.implementationLog, 'D242 is now next', 'Implementation log must mark D242 next.');
...
const d240 = tasks.find((task) => task.seq === 'D242');
requireTrue(d239?.status === 'beta', 'Progress dashboard task D239 must be beta after commit policy ships.');
requireTrue(d240?.status === 'next', 'Progress dashboard task D242 must become next after D239 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D242', 'Progress dashboard next queue must prioritize D242 after D239 ships.');
