...
requireIncludes(files.backlog, '| D241 |', 'Backlog must add D241 after D239.');
...
const d240 = tasks.find((task) => task.seq === 'D241');
requireTrue(d239?.status === 'beta', 'Progress dashboard task D239 must be beta after commit policy ships.');
requireTrue(d240?.status === 'next', 'Progress dashboard task D241 must become next after D239 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D241', 'Progress dashboard next queue must prioritize D241 after D239 ships.');
