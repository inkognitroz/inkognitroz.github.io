...
// image-boundary.js and vision-input.js were deleted: they were never loaded by any page,
// and neither the gateway nor the backend carries a vision route (backend /health capabilities:
// identity, memory, consent, proxy.models, proxy.chat_completions — measured 2026-09-22 16:43Z).
// The gate they used to document is now enforced by their absence.
for (const name of ['image-boundary.js', 'vision-input.js']) {
  if (existsSync(join(portalDir, name))) {
    fail(`${name} must stay deleted until a vision route is proven; reintroducing it needs its gates and a loading decision.`);
  }
}
