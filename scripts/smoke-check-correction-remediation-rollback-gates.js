...
requireIncludes(files.backlog, '| D252 |', 'Backlog must add D252 after D241.');
...
const d242 = tasks.find((task) => task.seq === 'D252');
requireTrue(d241?.status === 'beta', 'Progress dashboard task D241 must be beta after rollback gates ship.');
requireTrue(d242?.status === 'next', 'Progress dashboard task D252 must become next after D241 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D252', 'Progress dashboard next queue must prioritize D252 after D241 ships.');
