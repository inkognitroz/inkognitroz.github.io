...
requireIncludes(files.backlog, '| D243 |', 'Backlog must add D243 after D241.');
...
const d242 = tasks.find((task) => task.seq === 'D243');
requireTrue(d241?.status === 'beta', 'Progress dashboard task D241 must be beta after rollback gates ship.');
requireTrue(d242?.status === 'next', 'Progress dashboard task D243 must become next after D241 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D243', 'Progress dashboard next queue must prioritize D243 after D241 ships.');
