...
async function browserProof(){
  ...
  try{
    ...
    await page.addInitScript(()=>{window.MMIR_BACKEND_URL='https://backend.mmir.ai';});
    ...
    await page.route('**/*',async route=>{
      ...
      if(path==='/consent'){if(mode==='malformed'){mode='normal';return route.fulfill({status:200,contentType:'application/json',body:'{}'});}return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({object:'consent',memory:consent})});}
      if(path==='/memory'&&method==='POST'){memoryCalls+=1;if(mode==='fail-save'){mode='normal';return route.fulfill({status:503,contentType:'application/json',body:JSON.stringify({error:{code:'storage_unavailable'}})});}if(!consent)return route.fulfill({status:403,contentType:'application/json',body:JSON.stringify({error:{code:'consent_required'}})});const input=JSON.parse(route.request().postData()||'{}');const id='memory-fixture-'+(++nextId);records.set(id,{id,type:input.type,text:input.text,tags:[],created_at:now,updated_at:now});return route.fulfill({status:201,contentType:'application/json',body:JSON.stringify({object:'memory.item',data:records.get(id)})});}
