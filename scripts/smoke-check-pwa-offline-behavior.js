...
async function checkEarlyInstallPrompt(root,fail){
  ...
  if(window.__registeredServiceWorkerUrl!=='./sw.js?v=20260909-ordinary-chat-v1')fail('PWA update: deferred registration did not bind the ordinary-chat service-worker version.');
...
async function checkOfflineFetchBehavior(root,fail){
  ...
  const expectedCacheName='mmir-pwa-d358-20260909-ordinary-chat-v1';
