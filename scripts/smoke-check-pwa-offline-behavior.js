...
async function checkEarlyInstallPrompt(root,fail){
  ...
  if(window.__registeredServiceWorkerUrl!=='./sw.js?v=20260909-single-writer-readiness-v1')fail('PWA update: deferred registration did not bind the gateway-contract service-worker version.');
...
async function checkOfflineFetchBehavior(root,fail){
  ...
  const expectedCacheName='mmir-pwa-d357-20260909-single-writer-readiness-v1';
