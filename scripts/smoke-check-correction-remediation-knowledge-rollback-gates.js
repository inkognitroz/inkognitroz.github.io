...
requireIncludes(files.backlog, '| D246 |', 'Backlog must add D246 after D244.');
...
const d245 = tasks.find((task) => task.seq === 'D246');
requireTrue(d244?.status === 'beta', 'Progress dashboard task D244 must be beta after knowledge rollback ships.');
requireTrue(d245?.status === 'next', 'Progress dashboard task D246 must become next after D244 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D246', 'Progress dashboard next queue must prioritize D246 after D244 ships.');
