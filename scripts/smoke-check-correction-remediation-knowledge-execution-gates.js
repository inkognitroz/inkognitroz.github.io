...
requireIncludes(files.backlog, '| D247 |', 'Backlog must add D247 after D243.');
...
const d244 = tasks.find((task) => task.seq === 'D247');
requireTrue(d243?.status === 'beta', 'Progress dashboard task D243 must be beta after knowledge execution ships.');
requireTrue(d244?.status === 'next', 'Progress dashboard task D247 must become next after D243 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D247', 'Progress dashboard next queue must prioritize D247 after D243 ships.');
