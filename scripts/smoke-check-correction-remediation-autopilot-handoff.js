...
requireIncludes(files.backlog, '| D252 |', 'Backlog must add D252 after D246.');
...
const d247 = tasks.find((task) => task.seq === 'D252');
requireTrue(d246?.status === 'beta', 'Progress dashboard task D246 must be beta after handoff ships.');
requireTrue(d247?.status === 'next', 'Progress dashboard task D252 must become next after D246 ships.');
requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D252', 'Progress dashboard next queue must prioritize D252 after D246 ships.');
