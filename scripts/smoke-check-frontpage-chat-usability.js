...
requireIncludes(html, 'mimir-chat-portal.css?v=20260525-front-chat-order-v1', 'MMIR frontpage must ship the fresh chat order CSS cache key.');
requireIncludes(sw, "CACHE_NAME='mmir-pwa-d223-20260525-front-chat-order-v1'", 'Service worker cache must be bumped for the frontpage usability fix.');

for (const needle of [
  '.mimir-chat-first .mimir-greeting{order:1}',
  '.mimir-chat-first .mimir-composer{order:2}',
  '.mimir-chat-first #runtime-context-controls,.mimir-chat-first #mimir-chat-runtime{order:3}',
  '.mimir-chat-first .quick-suggestions{order:4}',
  '.mimir-chat-first #mmir-active-nodes-bar{order:5}',
...
requireBefore(css, '.mimir-chat-first .mimir-composer{order:2}', '.mimir-chat-first #runtime-context-controls,.mimir-chat-first #mimir-chat-runtime{order:3}', 'Composer must remain before live runtime.');
requireBefore(css, '.mimir-chat-first #runtime-context-controls,.mimir-chat-first #mimir-chat-runtime{order:3}', '.mimir-chat-first .quick-suggestions{order:4}', 'Live runtime must stay before secondary quick actions.');
requireBefore(css, '.mimir-chat-first .quick-suggestions{order:4}', '.mimir-chat-first #mmir-active-nodes-bar{order:5}', 'Quick prompts must stay above operational node details.');
