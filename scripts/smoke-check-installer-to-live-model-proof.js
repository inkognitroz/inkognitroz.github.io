...
async function run() {
  ...
  requireIncludes(files.backlog, '| D207 |', 'Backlog must keep a next sequential work item after D206.');
  ...
  requireTrue(d207?.status === 'next', 'Progress dashboard task D207 must become the next work item after D206 ships.');
  requireTrue(Array.isArray(progress.next_queue) && progress.next_queue[0] === 'D207', 'Progress dashboard next queue must prioritize D207 after D206 ships.');
