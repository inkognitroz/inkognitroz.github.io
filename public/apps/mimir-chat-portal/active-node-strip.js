(function(){
  ...
  function emitActiveRouteFeedbackCaptured(best,freshness,key,channel){
    const detail={
      ...
      route_freshness_state:String(freshness?.state||'unknown'),
      route_freshness_label:String(freshness?.label||''),
  ...
  function feedbackFooter(best,freshness){
    ...
    const canReport=freshness.state==='degraded'||manifestRefreshState==='failed';
    const feedbackAction=canReport?'<button type="button" data-active-route-feedback data-captured="'+safe(captured?'true':'false')+'" '+(captured?'disabled aria-disabled="true"':'')+'>'+safe(captured?'Route issue saved':'Report route issue')+'</button>':'';
    const status=captured?'Saved to Feedback Inbox for this route state. Recheck inventory before creating another route issue.':(canReport?'No route feedback saved for this visible state yet.':'Recheck route inventory first; create feedback only if refresh fails or the route is degraded.');
    return '<div class="mmir-active-node-overflow" data-route-feedback-state="'+safe(freshness.state)+'" data-route-feedback-captured="'+safe(captured?'true':'false')+'" data-route-feedback-action="'+safe(canReport?'report':'refresh-first')+'"><p>Capture route friction from this exact surface so owner triage keeps the live demo path honest. Recheck the public route manifest before escalating so demo trust can recover from this exact surface.</p><div class="mmir-active-node-overflow-actions"><button type="button" class="secondary" data-active-route-refresh aria-busy="'+safe(manifestRefreshState==='refreshing'?'true':'false')+'">'+safe(refreshButtonLabel(freshness))+'</button>'+feedbackAction+'</div><small class="mmir-active-route-feedback-status">'+safe(status)+'</small></div>';
  }
  ...
  function fallbackManifestNodes(){return [
    {id:'managed-api-bootstrap',name:FALLBACK_LABEL,type:'managed-api',status:'verify_before_chat',trust_level:'public-free',cost:{mode:'free',requires_approval:false},route:{kind:'managed-api',starter_id:'mmir-supergenius',url:'https://api.mmir.ai'},receipt_metadata:{node_type:'managed-api',trust_class:'public-free',cost_class:'free',quality_tier:'starter',execution_boundary:'public-free-route',prompt_left_device:false,provider_key_required:false,cloudflare_required:false,install_required:false},capabilities:['health','chat.completions','routing.free'],models:[{id:'mmir-supergenius',name:FALLBACK_LABEL,status:'available',provider:'mmir',source:'fallback-manifest'}]},
