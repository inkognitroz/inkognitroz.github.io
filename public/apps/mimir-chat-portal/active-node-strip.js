(function(){
  ...
  function routeInventoryFreshness(updatedAt){
    if(!updatedAt)return {state:'watch',label:'Route inventory freshness unknown',summary:'No updated_at field is present yet.'};
    const parsed=Date.parse(updatedAt);
    if(Number.isNaN(parsed))return {state:'watch',label:'Route inventory freshness unknown',summary:'The updated_at value is unreadable.'};
    const ageDays=(Date.now()-parsed)/86400000;
    if(ageDays>21)return {state:'degraded',label:'Route inventory stale',summary:'Refresh before demo trust. Updated '+formatAgeDays(ageDays)+'.'};
    if(ageDays>7)return {state:'watch',label:'Route inventory aging',summary:'Refresh soon. Updated '+formatAgeDays(ageDays)+'.'};
    return {state:'online',label:'Route inventory current',summary:'Updated '+formatAgeDays(ageDays)+'.'};
  }
  function nextStepAction(best,nodes){
    ...
    if(localNode&&!localReady())return {kind:'node',nodeId:'local-node',label:'Install Local Node',summary:'Next best step: connect the private local path so MMIR can upgrade from instant fallback to verified device-owned chat.'};
  ...
  function nextStepMarkup(action){
    ...
    return '<div class="mmir-active-node-next-step"><span>Next best step</span><p>'+safe(action.summary)+'</p>'+cta+'</div>';
  }
  async function loadManifest(){if(manifestLoaded)return;try{const res=await fetch(MANIFEST_URL,{cache:'default'});if(!res.ok)throw new Error('manifest unavailable');const body=await res.json();manifestUpdatedAt=String(body?.updated_at||'');manifestNodes=Array.isArray(body.nodes)?body.nodes.filter(node=>node?.id).map(normalizeNode):[];}catch(error){manifestUpdatedAt='';manifestNodes=[{id:'managed-api-bootstrap',name:FALLBACK_LABEL,route:{kind:'managed-api',starter_id:'mmir-supergenius',url:'https://api.mmir.ai'},models:[{name:FALLBACK_LABEL}]},{id:'local-node',name:'MMIR Local Node',route:{kind:'local-node',url:DEFAULT_LOCAL_URL},models:[]}];}manifestLoaded=true;}
