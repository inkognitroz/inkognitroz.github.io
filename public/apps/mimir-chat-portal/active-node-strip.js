(function(){
  ...
  function supergeniousLabel(value){return String(value||'').replace(/MMIR Browser Guide|MMIR Guide/g,'Supergenious');}
  function normalizeModel(model){if(!model||typeof model!=='object')return model;return {...model,name:supergeniousLabel(model.name),label:supergeniousLabel(model.label)};}
  function normalizeNode(node){if(!node||typeof node!=='object')return node;return {...node,name:supergeniousLabel(node.name),models:Array.isArray(node.models)?node.models.map(normalizeModel):node.models};}
  function normalizeStarter(model){
    if(!model||typeof model!=='object')return model;
    const isGuide=model.id==='mmir-guide'||model.runtime==='browser-guide'||/MMIR Browser Guide|MMIR Guide/i.test(String(model.label||''));
    if(!isGuide)return normalizeModel(model);
    return {...model,label:'Supergenious - instant free fallback',best_for:'Instant first answer while MMIR upgrades to a browser, API or local model route.',install_note:'No install required. Supergenious is the instant free fallback. MMIR automatically upgrades to WebGPU, api.mmir.ai or Local Node when available.'};
  }

  function selectedModel(){const select=q('#runtime-model'),option=select?.selectedOptions?.[0];return {value:select?.value||'',label:supergeniousLabel(String(option?.textContent||select?.value||'Supergenious').replace(/\s+-\s+live$/i,'').trim()),runtime:option?.dataset?.runtime||''};}
  ...
  function modelFromNode(node){const model=Array.isArray(node?.models)?node.models[0]:null;return supergeniousLabel(model?.name||model?.id||'Auto');}
  ...
  async function loadManifest(){if(manifestLoaded)return;try{const res=await fetch(MANIFEST_URL,{cache:'default'});if(!res.ok)throw new Error('manifest unavailable');const body=await res.json();manifestNodes=Array.isArray(body.nodes)?body.nodes.filter(node=>node?.id).map(normalizeNode):[];}catch(error){manifestNodes=[{id:'browser-guide',name:'Supergenious',route:{starter_id:'mmir-guide'},models:[{name:'Supergenious'}]},{id:'local-node',name:'MMIR Local Node',route:{kind:'local-node',url:DEFAULT_LOCAL_URL},models:[]}];}manifestLoaded=true;}
  function fallbackStarters(){return [{id:'mmir-guide',label:'Supergenious',runtime:'browser-guide',model:''},{id:'ollama-qwen3-06b',label:'Qwen3 0.6B',runtime:'ollama',model:'qwen3:0.6b'}];}
  ...
  function bestNode(nodes,selected){
    ...
    if((selected.runtime==='live'||selected.value==='mmir-guide')&&/Supergenious|MMIR Browser Guide|mmir-guide|MMIR Guide/i.test(label))return nodes.find(node=>node.id==='managed-api-bootstrap')||nodes.find(node=>node.id==='browser-guide')||nodes[0];
    if(selected.runtime==='browser-guide'||/Supergenious|MMIR Guide|Model Picker|Setup Coach|Security Coach|Growth Coach/i.test(label))return nodes.find(node=>node.id==='browser-guide')||nodes[0];
  ...
  function activateNode(node){
    ...
    selectStarter(node,'Start Supergenious instant free chat.');
  }
  ...
  function starterLabel(model){const prefix=model.runtime==='ollama'?'Install':(model.runtime==='webllm'?(webGpuReady()?'Browser Node':'Needs WebGPU/WASM'):'Now');return prefix+' '+supergeniousLabel(String(model.label||model.id||'model')).replace(/\s+-\s+.*$/,'');}
  function starterRail(){if(!starterModels.length)return '';const visible=starterModels.filter(model=>['browser-guide','webllm','ollama'].includes(model.runtime));if(!visible.length)return '';return '<div class="mmir-active-starter-rail" aria-label="Free starters" data-free-starter-count="'+String(visible.length)+'">'+visible.map(model=>'<button type="button" data-active-starter-id="'+safe(model.id)+'" data-starter-runtime="'+safe(model.runtime)+'" data-route-state="'+safe(starterState(model))+'" title="'+safe(model.install_note||model.best_for||model.label)+'">'+safe(starterLabel(model))+'</button>').join('')+'</div>';}
  function activateStarter(model){
    if(!model?.id)return;
    if(model.runtime==='webllm'&&!webGpuReady()){const guide=starterModels.find(item=>item.id==='mmir-guide')||{id:'mmir-guide'};handoff({starter_id:guide.id,action:'select',source:'active-node-starter-rail',fallback_for:model.id});const promptEl=q('#mimir-prompt');if(promptEl&&!String(promptEl.value||'').trim()){promptEl.value=(model.label||model.id)+' needs WebGPU/WASM. Show free local path.';promptEl.dispatchEvent(new Event('input',{bubbles:true}));promptEl.dispatchEvent(new Event('change',{bubbles:true}));}q('#primary-chat-link')?.click();return;}
  ...
  function render(){const composer=q('.mimir-composer');if(!composer)return;style();let bar=q('#mmir-active-nodes-bar');if(!bar){bar=d.createElement('section');bar.id='mmir-active-nodes-bar';bar.setAttribute('aria-label','Active chat routes');composer.parentNode.insertBefore(bar,composer.nextSibling);}const nodes=manifestNodes.length?manifestNodes:[{id:'browser-guide',name:'Supergenious',trust_level:'public-free',cost:{mode:'free'},route:{starter_id:'mmir-guide'},models:[{name:'Supergenious'}]}];const selected=selectedModel();const best=bestNode(nodes,selected);const state=nodeStatus(best),line=best.id==='local-node'&&localReady()?'Private local: '+nodeModel(best):'Free routes: '+(selected.label||'Supergenious');bar.dataset.state=state;bar.innerHTML='<div class="mmir-active-node-head"><div class="mmir-active-node-title"><span>Active chat routes</span><strong>'+safe(best.name)+'</strong><small>'+safe(line)+'</small></div><div class="mmir-active-node-pill">'+safe(state==='online'?'Ready':'Setup')+'</div></div>'+starterRail()+'<div class="mmir-active-node-grid">'+nodes.map(card).join('')+'</div>';q('#active-badge')&&(q('#active-badge').textContent='Active: '+best.name);q('#active-chat-title')&&(q('#active-chat-title').textContent=best.name+' active - chat now.');q('#active-chat-description')&&(q('#active-chat-description').textContent='Chat ready. Add local nodes anytime.');bar.querySelectorAll('[data-active-node-action]').forEach(button=>button.addEventListener('click',()=>activateNode(nodes.find(node=>node.id===button.getAttribute('data-active-node-action')))));bar.querySelectorAll('[data-active-starter-id]').forEach(button=>button.addEventListener('click',()=>activateStarter(starterModels.find(model=>model.id===button.getAttribute('data-active-starter-id')))));}
