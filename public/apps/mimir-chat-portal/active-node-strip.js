(function(){
  ...
  function normalizeStarter(model){
    ...
    return {...model,label:FALLBACK_LABEL,best_for:'Instant first answer while MMIR upgrades to a verified browser, API or local model route.',install_note:'No install required. Supergenious answers immediately. MMIR automatically upgrades to WebGPU, api.mmir.ai or Local Node when available.'};
  }
  ...
  function activateNode(node){
    ...
    selectStarter(node,'Start Supergenious instant chat.');
  }
  ...
  function activateStarter(model){
    if(!model?.id)return;
    if(model.runtime==='webllm'&&!webGpuReady()){const fallback=starterModels.find(item=>item.id==='mmir-supergenius')||{id:'mmir-supergenius'};handoff({starter_id:fallback.id,action:'select',source:'active-node-starter-rail',fallback_for:model.id});const promptEl=q('#mimir-prompt');if(promptEl&&!String(promptEl.value||'').trim()){promptEl.value='Browser Model needs WebGPU/WASM here. Use Supergenious now and show the private Local Model path.';promptEl.dispatchEvent(new Event('input',{bubbles:true}));promptEl.dispatchEvent(new Event('change',{bubbles:true}));}q('#primary-chat-link')?.click();return;}
