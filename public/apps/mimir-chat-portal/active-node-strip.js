(function(){
  ...
  function fallbackLabel(value){
    return String(value||'')
      .replace(/(?:\bMMIR\b\s*){2,}(?=Supergeni)/gi,'')
  ...
  async function loadManifest(){if(manifestLoaded)return;try{const res=await fetch(MANIFEST_URL,{cache:'default'});if(!res.ok)throw new Error('manifest unavailable');const body=await res.json();manifestNodes=Array.isArray(body.nodes)?body.nodes.filter(node=>node?.id).map(normalizeNode):[];}catch(error){manifestNodes=[{id:'browser-guide',name:FALLBACK_LABEL,route:{starter_id:'mmir-guide'},models:[{name:FALLBACK_LABEL}]},{id:'local-node',name:'MMIR Local Node',route:{kind:'local-node',url:DEFAULT_LOCAL_URL},models:[]}];}manifestLoaded=true;}
  function fallbackStarters(){return [{id:'mmir-guide',label:FALLBACK_LABEL,runtime:'browser-guide',model:''},{id:'ollama-qwen3-06b',label:'Qwen3 0.6B',runtime:'ollama',model:'qwen3:0.6b'}];}
