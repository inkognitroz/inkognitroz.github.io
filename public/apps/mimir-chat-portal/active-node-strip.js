(function(){
  ...
  function selectedModel(){const select=q('#runtime-model'),option=select?.selectedOptions?.[0];return {value:select?.value||'',label:fallbackLabel(String(option?.textContent||select?.value||'MMIR Supergenius').replace(/\s+-\s+live$/i,'').trim()),runtime:option?.dataset?.runtime||''};}
  ...
  async function loadManifest(){if(manifestLoaded)return;try{const res=await fetch(MANIFEST_URL,{cache:'default'});if(!res.ok)throw new Error('manifest unavailable');const body=await res.json();manifestNodes=Array.isArray(body.nodes)?body.nodes.filter(node=>node?.id).map(normalizeNode):[];}catch(error){manifestNodes=[{id:'browser-guide',name:'MMIR Supergenius',route:{starter_id:'mmir-guide'},models:[{name:'MMIR Supergenius'}]},{id:'local-node',name:'MMIR Local Node',route:{kind:'local-node',url:DEFAULT_LOCAL_URL},models:[]}];}manifestLoaded=true;}
  function fallbackStarters(){return [{id:'mmir-guide',label:'MMIR Supergenius',runtime:'browser-guide',model:''},{id:'ollama-qwen3-06b',label:'Qwen3 0.6B',runtime:'ollama',model:'qwen3:0.6b'}];}
  ...
  function bestNode(nodes,selected){
    ...
    if((selected.runtime==='live'||selected.value==='mmir-guide')&&/MMIR Supergenius|MMIR Browser Guide|mmir-guide|MMIR Guide/i.test(label))return nodes.find(node=>node.id==='managed-api-bootstrap')||nodes.find(node=>node.id==='browser-guide')||nodes[0];
    if(selected.runtime==='browser-guide'||/MMIR Supergenius|MMIR Guide|Model Picker|Setup Coach|Security Coach|Growth Coach/i.test(label))return nodes.find(node=>node.id==='browser-guide')||nodes[0];
  ...
  function activateNode(node){
    ...
    selectStarter(node,'Start MMIR Supergenius instant free chat.');
  }
  ...
  function starterLabel(model){const prefix=model.runtime==='ollama'?'Install':(model.runtime==='webllm'?(webGpuReady()?'Browser Node':'Needs WebGPU/WASM'):'Now');return prefix+' '+fallbackLabel(String(model.label||model.id||'model')).replace(/\s+-\s+.*$/,'');}
