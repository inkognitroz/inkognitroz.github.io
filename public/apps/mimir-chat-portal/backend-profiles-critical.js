(function(){
  ...
  function api(){const existing=read().find(p=>p.id==='mmir-api-bootstrap'||cl(p.url)===U);return up(p=>p.id==='mmir-api-bootstrap'||cl(p.url)===U,()=>({id:'mmir-api-bootstrap',createdAt:now()}),{id:'mmir-api-bootstrap',name:'MMIR Free Control Plane',url:U,provider:'openai-compatible',models:existing?.health==='ready'?'mmir-guide auto-discovered':'checked at runtime',keyRef:'no browser secret',cost:'free no paid routes',latency:'edge bootstrap',throughput:'bootstrap guide route',uptime:'cloudflare worker',health:existing?.health==='ready'?'ready':'unknown'})}
