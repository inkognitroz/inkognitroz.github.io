(function(){
  ...
  function modelState(sel,webGpu){const r=selectedRuntime(sel);if(r==='live')return 'ready';if(r==='webllm')return webGpu?'setup':'offline';if(r==='ollama')return 'setup';if(r==='browser-guide')return 'degraded';return 'idle';}
  function updateRuntime({modelSelect=q('runtime-model'),profile=null,webGpu=Boolean(navigator.gpu)}={}){
    ...
    chip('runtime-model-chip','Model: '+clip(m,38),modelState(modelSelect,webGpu),r==='browser-guide'?'MMIR Guide is a browser helper, not a live LLM. Choose WebGPU or Local Node for a real model.':m);
