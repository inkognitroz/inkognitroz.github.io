(function(){
  ...
  function updateRuntime({modelSelect=q('runtime-model'),profile=null,webGpu=Boolean(navigator.gpu)}={}){
    ...
    chip('runtime-model-chip',clip(m,38),modelState(modelSelect,webGpu),r==='auto'?'MMIR Supergenius is ready now on the hosted free route.':(r==='browser-guide'?'MMIR Guide works now as a free browser helper. Choose WebGPU or Local Node for a full local LLM.':m));
