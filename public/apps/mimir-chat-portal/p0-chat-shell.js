(function(){
  ...
  const P0_RUNTIME_VERSION='20260713-connected-intelligence-certainty-v4';
  ...
  function explicitGroundingInstruction(prompt){
    const asksForBasis=/\b(source|sources|citation|citations|evidence|basis|based on|grounded|what (?:is|are) your sources?|what (?:are|is) you basing (?:this|that|(?:your )?answer) on|what supports (?:this|that|it)|prove (?:this|that|it)|(?:can|could) you verify (?:this|that|it|the claim)|is (?:this|that|it) true|are you (?:(?:really )?sure|certain)(?: about (?:this|that|it))?(?=[?.!,;:]|$)|how certain are you(?: about (?:this|that|it))?(?=[?.!,;:]|$)|kilde|kilder|kilden(?: din)?|kildene(?: dine)?|henvisning|henvisninger|belegg|grunnlag|hva er (?:kilden din|kildene dine)|hva baserer du (?:det(?:te)?|svaret(?: ditt)?) på|hva støtter (?:det(?:te)?|(?:denne )?påstanden)|bevis (?:det(?:te)?|påstanden)|kan du verifisere (?:det(?:te)?|påstanden)|stemmer (?:det(?:te)?)|er (?:det(?:te)?) sant|er du (?:helt )?sikker(?: på det(?:te)?)?(?=[?.!,;:]|$)|hvor sikker er du(?: på det(?:te)?)?(?=[?.!,;:]|$)|bygger du (?:svaret(?: ditt)? )?på|basert på|hvor (?:har du (?:det(?:te)?|informasjonen) fra|kommer (?:det(?:te)?|informasjonen) fra|fant du (?:det(?:te)?|informasjonen))|hvordan vet du (?:det(?:te)?|dette her)|kan du dokumentere (?:det(?:te)?|dette her)|where (?:(?:is|was) (?:this|that|the information)|did you get (?:this|that|the information)|does (?:this|that|the information) come) from|how do you know (?:this|that)|can you back (?:this|that|it) up)(?=\s|[?.!,;:]|$)/i.test(String(prompt||''));
