(function(){
  ...
  const P0_RUNTIME_VERSION='20260712-norwegian-evidence-retry-v1';
  ...
  function explicitGroundingInstruction(prompt){
    const asksForBasis=/\b(source|sources|citation|citations|evidence|basis|based on|grounded|kilde|kilder|henvisning|henvisninger|belegg|grunnlag|bygger du (?:svaret(?: ditt)? )?på|basert på|hvor (?:har du (?:det(?:te)?|informasjonen) fra|kommer (?:det(?:te)?|informasjonen) fra)|hvordan vet du (?:det(?:te)?|dette her)|where did you get (?:this|that|the information) from|how do you know (?:this|that))(?=\s|[?.!,;:]|$)/i.test(String(prompt||''));
