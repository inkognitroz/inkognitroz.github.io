(function(){
  ...
  function renderCorrectionRemediationAutopilotRollbackReadinessReport(data){
    const report=data.correction_remediation_autopilot_rollback_readiness_report||{};
    const scenarios=Array.isArray(report.scenarios)?report.scenarios:[];
    const phases=Array.isArray(report.readiness_phases)?report.readiness_phases:[];
    if(!scenarios.length&&!phases.length)return '';
    const policy=report.readiness_policy||{};
    const ready=scenarios.every((scenario)=>scenario.status==='ready'&&(scenario.source_mutation_executed===false||scenario.public_frontend_authority===false||scenario.provider_secrets_stored===false||scenario.document_text_stored===false||scenario.no_paid_routes_started===true));
    return '<section id="progress-correction-remediation-autopilot-rollback-readiness" class="progress-no-model-fixture" data-state="'+(ready?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Rollback readiness</p><h2>'+safe(report.title||'Autopilot Rollback Readiness Cards')+'</h2></div><small>Make the undo path visible before and after protected source changes.</small></div>'+
      '<div class="progress-no-model-grid">'+
        (report.backend_route?'<article class="progress-no-model-scenario" data-route="'+safe(report.backend_route.method+' '+report.backend_route.path)+'"><span>'+safe(label(report.backend_route.status||'beta'))+'</span><h3>'+safe(report.backend_route.method+' '+report.backend_route.path)+'</h3><p>'+safe(report.backend_route.purpose||'Protected rollback readiness route.')+'</p><strong>'+safe(report.backend_route.auth||'protected-backend-auth')+'</strong><small>backend:'+safe(report.backend_commit||'tracked')+' / backend_only_rollback:'+safe(Boolean(policy.backend_only_rollback))+'</small></article>':'')+
        phases.map((phase)=>'<article class="progress-no-model-scenario" data-phase="'+safe(phase.phase)+'"><span>'+safe(label(phase.status||'ready'))+'</span><h3>'+safe(phase.phase||'readiness phase')+'</h3><p>'+safe(phase.expected||'Rollback readiness phase.')+'</p><strong>rollback_now:'+safe(Boolean(phase.rollback_available_now))+'</strong><small>rollback_after_execution:'+safe(Boolean(phase.rollback_available_after_execution))+' / source_mutation_executed:'+safe(Boolean(phase.source_mutation_executed))+'</small></article>').join('')+
        scenarios.map((scenario)=>'<article class="progress-no-model-scenario" data-scenario="'+safe(scenario.id)+'"><span>'+safe(label(scenario.status==='ready'?'ready':'watch'))+'</span><h3>'+safe(scenario.id)+'</h3><p>'+safe(scenario.expected||'Rollback readiness scenario.')+'</p><strong>'+safe(scenario.selector||'rollback readiness')+'</strong><small>document_text_stored:'+safe(Boolean(scenario.document_text_stored))+' / no_paid_routes_started:'+safe(Boolean(scenario.no_paid_routes_started))+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe rollback readiness evidence only.')+' Next: '+safe(report.next?.task||'D248')+' '+safe(report.next?.label||'autopilot trust timeline')+'.</small>'+
    '</section>';
  }

  function renderCorrectionRemediationAutopilotTrustTimelineReport(data){
    const report=data.correction_remediation_autopilot_trust_timeline_report||{};
    const scenarios=Array.isArray(report.scenarios)?report.scenarios:[];
    const steps=Array.isArray(report.timeline_steps)?report.timeline_steps:[];
    if(!scenarios.length&&!steps.length)return '';
    const policy=report.timeline_policy||{};
    const ready=scenarios.every((scenario)=>scenario.status==='ready'&&(scenario.public_frontend_authority===false||scenario.automatic_mutation_allowed===false||scenario.backend_only_rollback===true||scenario.provider_secrets_stored===false||scenario.document_text_stored===false||scenario.no_paid_routes_started===true));
    return '<section id="progress-correction-remediation-autopilot-trust-timeline" class="progress-no-model-fixture" data-state="'+(ready?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Guided trust timeline</p><h2>'+safe(report.title||'Autopilot Guided Trust Timeline')+'</h2></div><small>One lane from safe autopilot to explicit source confirmation and undo.</small></div>'+
      '<div class="progress-no-model-grid">'+
        (Array.isArray(report.uses_existing_routes)?report.uses_existing_routes.map((route)=>'<article class="progress-no-model-scenario" data-route="'+safe(route.method+' '+route.path)+'"><span>route</span><h3>'+safe(route.method+' '+route.path)+'</h3><p>'+safe(route.purpose||'Protected route.')+'</p><strong>'+safe(route.auth||'protected-backend-auth')+'</strong><small>backend-only</small></article>').join(''):'')+
        steps.map((step)=>'<article class="progress-no-model-scenario" data-step="'+safe(step.id)+'"><span>'+safe(step.expected_state||'watch')+'</span><h3>'+safe(step.label||step.id)+'</h3><p>'+safe(step.why||'Timeline step.')+'</p><strong>'+safe(step.id||'step')+'</strong><small>public_frontend_authority:'+safe(Boolean(policy.public_frontend_authority))+'</small></article>').join('')+
        scenarios.map((scenario)=>'<article class="progress-no-model-scenario" data-scenario="'+safe(scenario.id)+'"><span>'+safe(label(scenario.status==='ready'?'ready':'watch'))+'</span><h3>'+safe(scenario.id)+'</h3><p>'+safe(scenario.expected||'Trust timeline scenario.')+'</p><strong>'+safe(scenario.selector||'trust timeline')+'</strong><small>no_paid_routes_started:'+safe(Boolean(scenario.no_paid_routes_started))+' / backend_only_rollback:'+safe(Boolean(scenario.backend_only_rollback))+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe timeline evidence only.')+' Next: '+safe(report.next?.task||'D249')+' '+safe(report.next?.label||'autopilot trust timeline browser QA')+'.</small>'+
    '</section>';
  }

  function renderCorrectionRemediationAutopilotTrustTimelineBrowserQaReport(data){
    const report=data.correction_remediation_autopilot_trust_timeline_browser_qa_report||{};
    const fixture=data.correction_remediation_autopilot_trust_timeline_browser_fixture||{};
    const scenarios=Array.isArray(report.scenarios)?report.scenarios:[];
    const viewports=Array.isArray(report.viewports)?report.viewports:[];
    const fixtureStates=Array.isArray(fixture.states)?fixture.states:[];
    if(!scenarios.length&&!viewports.length)return '';
    const policy=report.qa_policy||{};
    const ready=scenarios.every((scenario)=>scenario.status==='ready')&&viewports.every((viewport)=>viewport.status==='ready');
    return '<section id="progress-correction-remediation-autopilot-trust-timeline-browser-qa" class="progress-no-model-fixture" data-state="'+(ready?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Timeline browser QA</p><h2>'+safe(report.title||'Autopilot Trust Timeline Browser QA')+'</h2></div><small>Desktop/mobile selector and visual contracts for the guided trust lane.</small></div>'+
      '<div class="progress-no-model-grid">'+
        viewports.map((viewport)=>'<article class="progress-no-model-scenario" data-viewport="'+safe(viewport.id)+'"><span>'+safe(viewport.status||'watch')+'</span><h3>'+safe(viewport.id)+' '+safe(viewport.width)+'x'+safe(viewport.height)+'</h3><p>'+safe((viewport.checks||[]).join(', ')||'Viewport QA')+'</p><strong>visual_overlap_guard:'+safe(Boolean(policy.visual_overlap_guard))+'</strong><small>desktop_mobile_coverage:'+safe(Boolean(policy.desktop_mobile_coverage))+'</small></article>').join('')+
        fixtureStates.map((state)=>'<article class="progress-no-model-scenario" data-fixture-state="'+safe(state.id)+'"><span>fixture</span><h3>'+safe(state.label||state.id)+'</h3><p>Expected panel state: '+safe(state.expected_panel_state||'watch')+'</p><strong>'+safe((state.expected_enabled_actions||[]).join(', ')||'no enabled destructive actions')+'</strong><small>no_paid_routes_started:'+safe(Boolean(fixture.privacy?.no_paid_routes_started))+'</small></article>').join('')+
        scenarios.map((scenario)=>'<article class="progress-no-model-scenario" data-scenario="'+safe(scenario.id)+'"><span>'+safe(label(scenario.status==='ready'?'ready':'watch'))+'</span><h3>'+safe(scenario.id)+'</h3><p>'+safe(scenario.expected||'Browser QA scenario.')+'</p><strong>'+safe((scenario.enabled_actions||[]).join(', ')||'none')+'</strong><small>public_frontend_authority:'+safe(Boolean(policy.public_frontend_authority))+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe browser fixture only.')+' Next: '+safe(report.next?.task||'D250')+' '+safe(report.next?.label||'autopilot timeline post-action receipts')+'.</small>'+
    '</section>';
  }

  function renderCorrectionRemediationAutopilotTimelineReceiptsReport(data){
    const report=data.correction_remediation_autopilot_timeline_receipts_report||{};
    const scenarios=Array.isArray(report.scenarios)?report.scenarios:[];
    const routes=Array.isArray(report.routes)?report.routes:[];
    if(!scenarios.length&&!routes.length)return '';
    const policy=report.receipt_policy||{};
    const ready=scenarios.every((scenario)=>scenario.status==='ready'&&(scenario.automatic_mutation_allowed===false||scenario.backend_only_rollback===true||scenario.provider_secrets_stored===false||scenario.document_text_stored===false||scenario.no_paid_routes_started===true));
    return '<section id="progress-correction-remediation-autopilot-timeline-receipts" class="progress-no-model-fixture" data-state="'+(ready?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Timeline receipts</p><h2>'+safe(report.title||'Autopilot Timeline Post-Action Receipts')+'</h2></div><small>Compact proof after confirm, refresh and undo actions.</small></div>'+
      '<div class="progress-no-model-grid">'+
        routes.map((route)=>'<article class="progress-no-model-scenario" data-action="'+safe(route.action)+'"><span>route</span><h3>'+safe(route.action)+'</h3><p>'+safe(route.route||route.memory_route||'protected route')+'</p><strong>next:'+safe(route.next_action||'review timeline')+'</strong><small>backend_required:'+safe(Boolean(policy.protected_backend_route_required))+'</small></article>').join('')+
        scenarios.map((scenario)=>'<article class="progress-no-model-scenario" data-scenario="'+safe(scenario.id)+'"><span>'+safe(label(scenario.status==='ready'?'ready':'watch'))+'</span><h3>'+safe(scenario.id)+'</h3><p>'+safe(scenario.expected||'Timeline receipt scenario.')+'</p><strong>'+safe(scenario.selector||'timeline receipt')+'</strong><small>no_paid_routes_started:'+safe(Boolean(scenario.no_paid_routes_started))+' / document_text_stored:'+safe(Boolean(scenario.document_text_stored))+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe receipt metadata only.')+' Next: '+safe(report.next?.task||'D251')+' '+safe(report.next?.label||'autopilot timeline receipt browser QA')+'.</small>'+
    '</section>';
  }

  function renderCorrectionRemediationAutopilotTimelineReceiptBrowserQaReport(data){
    const report=data.correction_remediation_autopilot_timeline_receipts_browser_qa_report||{};
    const fixture=data.correction_remediation_autopilot_timeline_receipts_browser_fixture||{};
    const scenarios=Array.isArray(report.scenarios)?report.scenarios:[];
    const viewports=Array.isArray(report.viewports)?report.viewports:[];
    const states=Array.isArray(fixture.states)?fixture.states:[];
    if(!scenarios.length&&!viewports.length&&!states.length)return '';
    const policy=report.qa_policy||{};
    const ready=scenarios.every((scenario)=>scenario.status==='ready')&&viewports.every((viewport)=>viewport.status==='ready');
    return '<section id="progress-correction-remediation-autopilot-timeline-receipts-browser-qa" class="progress-no-model-fixture" data-state="'+(ready?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Receipt browser QA</p><h2>'+safe(report.title||'Autopilot Timeline Receipt Browser QA')+'</h2></div><small>Desktop/mobile fixture proof for running, ready and error receipts.</small></div>'+
      '<div class="progress-no-model-grid">'+
        viewports.map((viewport)=>'<article class="progress-no-model-scenario" data-viewport="'+safe(viewport.id)+'"><span>'+safe(viewport.status||'watch')+'</span><h3>'+safe(viewport.id)+' '+safe(viewport.width)+'x'+safe(viewport.height)+'</h3><p>'+safe((viewport.checks||[]).join(', ')||'Viewport QA')+'</p><strong>visual_overlap_guard:'+safe(Boolean(policy.visual_overlap_guard))+'</strong><small>desktop_mobile_coverage:'+safe(Boolean(policy.desktop_mobile_coverage))+'</small></article>').join('')+
        states.map((state)=>'<article class="progress-no-model-scenario" data-receipt-state="'+safe(state.id)+'"><span>'+safe(state.expected_receipt_state||'watch')+'</span><h3>'+safe(state.label||state.id)+'</h3><p>'+safe(state.expected_route||'protected route')+'</p><strong>'+safe(state.expected_action||'timeline action')+' -> '+safe(state.expected_next_action||'review timeline')+'</strong><small>result:'+safe(state.expected_result_status||'pending')+' / no_paid_routes_started:'+safe(Boolean(fixture.privacy?.no_paid_routes_started))+'</small></article>').join('')+
        scenarios.map((scenario)=>'<article class="progress-no-model-scenario" data-scenario="'+safe(scenario.id)+'"><span>'+safe(label(scenario.status==='ready'?'ready':'watch'))+'</span><h3>'+safe(scenario.action||scenario.id)+'</h3><p>'+safe(scenario.expected||'Receipt browser QA scenario.')+'</p><strong>'+safe(scenario.selector||'receipt selector')+'</strong><small>public_frontend_authority:'+safe(Boolean(policy.public_frontend_authority))+' / automatic_mutation_allowed:'+safe(Boolean(policy.automatic_mutation_allowed))+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe receipt fixture only.')+' Next: '+safe(report.next?.task||'D252')+' '+safe(report.next?.label||'cross-repo architecture and security review gate')+'.</small>'+
    '</section>';
  }

  function renderCrossRepoArchitectureSecurityReviewReport(data){
    const report=data.cross_repo_architecture_security_review_report||{};
    const evidence=Array.isArray(report.evidence)?report.evidence:[];
    const findings=Array.isArray(report.code_review_findings)?report.code_review_findings:[];
    const scorecard=Array.isArray(report.architecture_scorecard)?report.architecture_scorecard:[];
    if(!evidence.length&&!findings.length&&!scorecard.length)return '';
    const allCriticalGreen=findings.filter((finding)=>finding.severity==='P0').every((finding)=>finding.status==='pass');
    return '<section id="progress-cross-repo-architecture-security-review" class="progress-no-model-fixture" data-state="'+(allCriticalGreen?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Code + architecture review</p><h2>'+safe(report.title||'Cross-Repo Architecture and Security Review Gate')+'</h2></div><small>Public-safe review evidence across frontend, backend, zero-trust and UX.</small></div>'+
      '<div class="progress-no-model-grid">'+
        evidence.map((item)=>'<article class="progress-no-model-scenario" data-evidence="'+safe(item.id)+'"><span>'+safe(item.status||'watch')+'</span><h3>'+safe(item.repo||'repo')+'</h3><p>'+safe(item.result||'Review evidence')+'</p><strong>'+safe(item.scope||'scope')+'</strong><small>id:'+safe(item.id||'evidence')+'</small></article>').join('')+
        scorecard.map((item)=>'<article class="progress-no-model-scenario" data-area="'+safe(item.area)+'"><span>'+safe(item.status||'watch')+'</span><h3>'+safe(item.area||'architecture')+'</h3><p>'+safe(item.finding||'Architecture finding')+'</p><strong>'+safe(item.next_action||'review next action')+'</strong><small>zero_trust:true</small></article>').join('')+
        findings.map((finding)=>'<article class="progress-no-model-scenario" data-finding="'+safe(finding.id)+'"><span>'+safe(finding.severity||'P1')+' '+safe(finding.status||'watch')+'</span><h3>'+safe(finding.id||'finding')+'</h3><p>'+safe(finding.finding||'Code review finding')+'</p><strong>'+safe(finding.next_action||'next action')+'</strong><small>evidence:'+safe((finding.evidence||[]).join(', ')||'review')+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe review metadata only.')+' Security: '+safe(report.security_review?.status||'watch')+' / UX: '+safe(report.ux_review?.status||'watch')+' / no_spend:'+safe(Boolean(report.no_spend?.paid_services_used===false))+'. Next: '+safe(report.next?.task||'D253')+' '+safe(report.next?.label||'end-to-end free activation canary')+'.</small>'+
    '</section>';
  }

  function renderChatFirstFreeActivationCanaryReport(data){
    const report=data.chat_first_free_activation_canary_report||{};
    const path=Array.isArray(report.canary_path)?report.canary_path:[];
    const journeys=Array.isArray(report.journeys)?report.journeys:[];
    const evidence=Array.isArray(report.evidence)?report.evidence:[];
    if(!path.length&&!journeys.length&&!evidence.length)return '';
    const p0Ready=journeys.filter((journey)=>journey.priority==='P0').every((journey)=>/^ready|^beta/.test(String(journey.status||'')));
    return '<section id="progress-chat-first-free-activation-canary" class="progress-no-model-fixture" data-state="'+(p0Ready?'ready':'watch')+'">'+
      '<div class="progress-route-map-head"><div><p class="eyebrow">Chat-first activation canary</p><h2>'+safe(report.title||'Chat-First Free Activation Canary')+'</h2></div><small>'+safe(report.principle||'Type first, configure later.')+'</small></div>'+
      '<div class="progress-no-model-grid">'+
        path.map((step)=>'<article class="progress-no-model-scenario" data-canary-step="'+safe(step.id)+'"><span>'+safe(label(step.status||'watch'))+'</span><h3>'+safe(step.surface||step.id)+'</h3><p>'+safe(step.expected||'Canary step')+'</p><strong>'+safe(step.selector||'selector')+'</strong><small>step:'+safe(step.id||'canary')+'</small></article>').join('')+
        journeys.map((journey)=>'<article class="progress-no-model-scenario" data-journey="'+safe(journey.id)+'"><span>'+safe(journey.priority||'P1')+' '+safe(label(journey.status||'watch'))+'</span><h3>'+safe(journey.name||journey.id)+'</h3><p>'+safe(journey.user_goal||'User journey')+'</p><strong>'+safe(journey.automatic_path||'automatic path')+'</strong><small>'+safe(journey.completion_signal||'completion signal')+'</small></article>').join('')+
        evidence.map((item)=>'<article class="progress-no-model-scenario" data-evidence="'+safe(item.id)+'"><span>'+safe(item.status||'watch')+'</span><h3>'+safe(item.scope||'evidence')+'</h3><p>'+safe(item.result||'Evidence')+'</p><strong>'+safe(item.id||'evidence')+'</strong><small>no_spend:'+safe(Boolean(report.no_spend?.paid_services_used===false))+'</small></article>').join('')+
      '</div>'+
      '<small class="progress-activation-privacy">'+safe(report.public_repo_rule||'Public-safe canary metadata only.')+' Result: '+safe(report.result||'watch')+'. Next: '+safe(report.next?.task||'D254')+' '+safe(report.next?.label||'chat-first polish')+'.</small>'+
    '</section>';
  }
